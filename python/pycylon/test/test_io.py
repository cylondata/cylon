...
#     df = DataFrame.read_csv(input_files, env=env)
...
#         df2 = DataFrame.read_csv(paths={env.rank: out_file}, env=env)
...
#     df = DataFrame.read_json(input_files, env=env, lines=True)
...
#         df2 = DataFrame.read_json(paths={env.rank: out_file}, env=env, lines=True)
#         assert len(df) == len(df2), \
...
#     df = DataFrame.read_parquet(input_files, env=env)
#     assert len(df) == rows_per_worker[env.rank], \
...
#     df = DataFrame.read_parquet(input_dir, env=env)
#     assert len(df) == rows_per_worker[env.rank], \
#         f'Read Parquet DataFrame row count [{len(df)}] does not match the given row count [{rows_per_worker[env.rank]}]'
...
#         df2 = DataFrame.read_parquet(paths={env.rank: out_file}, env=env)
#         assert len(df) == len(df2), \

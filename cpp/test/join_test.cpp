...
TEST_CASE("Multi Index Join testing", "[multi_join]") {
   ...
   std::string out_path = "../data/output/multi_join.csv";

   SECTION("testing inner joins - sort") {
     const auto &join_config = join::config::JoinConfig::InnerJoin({0,1}, {0,1}, cylon::join::config::JoinAlgorithm::SORT);
     REQUIRE(test::TestJoinOperation(join_config, ctx, path1, path2, out_path) == 0);
   }

   SECTION("testing inner joins - hash") {
     const auto &join_config = join::config::JoinConfig::InnerJoin({0,1}, {0,1}, cylon::join::config::JoinAlgorithm::HASH);
     REQUIRE(test::TestJoinOperation(join_config, ctx, path1, path2, out_path) == 0);
   }
}

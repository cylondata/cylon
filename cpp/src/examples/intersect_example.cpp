...
int main(int argc, char *argv[]) {
  ...
  if (status1.is_ok() && status2.is_ok()) {
    ...
    LOG(INFO) << "Table 1 had : " << table1->Rows() << " and Table 2 had : " << table2->Rows() << ", Intersect has : "
              << result->Rows();
    result->Print();
    LOG(INFO) << "Intersect done in " << std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count() << "[ms]";
  } else {

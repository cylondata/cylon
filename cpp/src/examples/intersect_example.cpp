...
int main(int argc, char *argv[]) {
  if (argc < 5) {
    LOG(ERROR) << "./intersect_example m [n | o] num_tuples_per_worker 0.0-1.0" << std::endl
               << "./intersect_example m [n | o] num_tuples_per_worker 0.0-1.0" << std::endl
               << "./intersect_example f [n | o] csv_file1 csv_file2" << std::endl
               << "./intersect_example f [n | o] csv_file1 csv_file2" << std::endl;
    return 1;
  }
  ...
  if (ops_param == "o") {
    status = cylon::CCDistributedIntersect(ctx, first_table, second_table, unioned_table);
  } else if (ops_param == "n") {
    status = cylon::DistributedIntersect(first_table, second_table, unioned_table);
  } else {

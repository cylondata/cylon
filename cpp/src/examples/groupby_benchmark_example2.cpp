...
void PipeCylonGroupBy(int i, arrow::MemoryPool *pool, std::shared_ptr<cylon::Table> &ctable,
                      std::shared_ptr<cylon::Table> &output) {
  auto t1 = std::chrono::steady_clock::now();
...
/* // using hashgroup by template function
  ...
  auto t3 = std::chrono::steady_clock::now();
  LOG(INFO) << "pipe_group3 iter "<< i << " rank " << ctable->GetContext()->GetRank()
            << " world " << ctable->GetContext()->GetWorldSize()
            << " res " << output->Rows()
            << " time " << std::chrono::duration_cast<std::chrono::milliseconds>(t3 - t1).count()
            << std::endl;
...
//  first_table->WriteCSV("/tmp/source" + std::to_string(ctx->GetRank()) + ".txt");
  ...
  std::shared_ptr<cylon::Table> output, input;
...
/*  // naive group by
  ...
  status = cylon::Sort(first_table, 0, input);
  if (!status.is_ok()) {
    LOG(INFO) << "Table sorting failed " << argv[1];
    ctx->Finalize();
    return 1;
  }

  for (int i = 0; i < std::stoi(argv[4]); ++i){
    HashCylonGroupBy(i, pool, input, output, use_local);
    output.reset();
    PipeCylonGroupBy(i, pool, input, output);
    output.reset();
  }
//  output->Print();

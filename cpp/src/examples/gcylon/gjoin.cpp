...
int main(int argc, char *argv[]) {
  ...
  auto tv1 = tbl1->view();
  LOG(INFO) << "my_rank: " << my_rank << ", initial dataframe. cols: " << tv1.num_columns() << ", rows: "
            << tv1.num_rows();
  printWholeTable(tv1);
  ...
  auto tv2 = tbl2->view();
  LOG(INFO) << "my_rank: " << my_rank << ", initial dataframe. cols: " << tv2.num_columns() << ", rows: "
            << tv2.num_rows();
  printWholeTable(tv2);
  ...
  if (tvj.num_rows() == 0) {
    ...
    LOG(INFO) << my_rank << ", joined table. number of columns: " << tvj.num_columns() << ", number of rows: "
              << tvj.num_rows();
    printWholeTable(tvj);
  }

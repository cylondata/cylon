...
#include <cylon/util/PackString.hpp>
...
namespace cylon {
    namespace net {
        ...
        Status UCXCommunicator::MakeOOB(const std::shared_ptr<CommConfig> &config, MemoryPool *pool,
            ...
            std::shared_ptr<UCXSocketData> socketData;
            ...
            *out = *ucxComm;
            ...
            int addrSize = ucpRecvWorkerAddr->addrSize;


            std::unique_ptr<uint8_t[]> allAddresses;

            uint8_t * addr = nullptr;



            if (ucxComm->get()->getAddressTyp() != UCX_ADDRESS_TYP::UCP) {
                addrSize = sizeof(UCXSocketData);

                util::PackString packString;
                auto scktdta = socketData.get();
                packString.writeObject(scktdta);

                addr = std::make_shared<uint8_t>().get();

                std::copy(packString.bytes(), packString.bytesEnd(), addr);

            } else {
                addr = (uint8_t *) ucpRecvWorkerAddr->addr;
            }

            allAddresses = std::make_unique<uint8_t[]>(addrSize * world_size);
            ...
            RETURN_CYLON_STATUS_IF_FAILED(oob_context->OOBAllgather(
                    /*(uint8_t *) ucpRecvWorkerAddr->addr*/addr, allAddresses.get(),
                    /*(int) ucpRecvWorkerAddr->addrSize*/ addrSize,
                    /*(int) ucpRecvWorkerAddr->addrSize*/addrSize));
            ...
            for (sIndx = 0; sIndx < world_size; sIndx++) {
                ...
                if (rank != sIndx) {
                    reinterpret_cast<ucp_address_t *>(
...
//    RETURN_CYLON_STATUS_IF_UCC_FAILED(ucc_context_progress(comm.uccContext));
        ...
        const std::string &UCXSocketData::getAddress() const {
            return address;
        }

        int UCXSocketData::getPort() const {
            return port;
        }

        UCXSocketData::UCXSocketData(std::string address, int port) : address(address), port(port) {}
    }  // namespace net
}  // namespace cylon

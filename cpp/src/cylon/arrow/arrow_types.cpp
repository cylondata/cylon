...
std::shared_ptr<arrow::DataType> ToArrowType(const std::shared_ptr<DataType> &type) {
  switch (type->getType()) {
    ...
    case Type::FIXED_SIZE_BINARY:
      return arrow::fixed_size_binary(
          std::static_pointer_cast<FixedSizeBinaryType>(type)->byte_width_);
    case Type::DATE32:
    ...
    case Type::LARGE_STRING:
      return std::make_shared<arrow::LargeStringType>();
    case Type::LARGE_BINARY:
      return std::make_shared<arrow::LargeBinaryType>();
    case Type::DECIMAL: {
      const auto &casted = std::static_pointer_cast<DecimalType>(type);
      if (casted->byte_width_ == 16)
        return arrow::decimal128(casted->precision_, casted->scale_);
      ...
      case arrow::Type::LIST: {
        ...
        switch (t_value->value_type()->id()) {
          ...
          case arrow::Type::DOUBLE:
            continue;
          default:
            return {Code::NotImplemented, "unsupported value type for lists " +
                                              t_value->value_type()->ToString()};
            ;
        }
      }
      default:
        return {Code::NotImplemented, "unsupported type " + t->type()->ToString()};
    }
  }
  return Status::OK();
}
...
std::shared_ptr<DataType> ToCylonType(const std::shared_ptr<arrow::DataType> &a_type) {
  switch (a_type->id()) {
    ...
    case arrow::Type::INT64:
      return cylon::Int64();
    case arrow::Type::HALF_FLOAT:
      return cylon::HalfFloat();
    case arrow::Type::FLOAT:
      return cylon::Float();
    case arrow::Type::DOUBLE:
      return cylon::Double();
    case arrow::Type::FIXED_SIZE_BINARY:
      return cylon::FixedSizeBinary(
          std::static_pointer_cast<arrow::FixedSizeBinaryType>(a_type)->byte_width());
    case arrow::Type::BINARY:
      return cylon::Binary();
    case arrow::Type::STRING:
      return cylon::String();
    case arrow::Type::LARGE_STRING:
      return cylon::LargeString();
    case arrow::Type::LARGE_BINARY:
      return cylon::LargeBinary();
    case arrow::Type::DATE32:
      return cylon::Date32();
    case arrow::Type::DATE64:
      return cylon::Date64();

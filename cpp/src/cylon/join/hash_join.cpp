...
arrow::Status ArrayIndexHashJoin(const std::shared_ptr<arrow::Array> &left_idx_col,
  ...
  std::array<std::vector<uint32_t>, 2>
      hashes{std::vector<uint32_t>(left_idx_col->length(), 0), std::vector<uint32_t>(right_idx_col->length(), 0)};
  hash_kernel->UpdateHash(left_idx_col, hashes[0]); // calc hashes of left array
  hash_kernel->UpdateHash(right_idx_col, hashes[1]); // calc hashes of right array
  ...
  // comparator
  const auto &comp = CreateArrayIndexComparator(left_idx_col, right_idx_col);
  ...
  hashes[build_idx].clear(); // hashes of the building array are no longer needed
  ...
  std::cout << "idx hash join: setup: " << std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count()
            ...
            << " total: " << std::chrono::duration_cast<std::chrono::milliseconds>(t4 - t1).count() << std::endl;

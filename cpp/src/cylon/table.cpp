...
static inline Status prepare_array(std::shared_ptr<cylon::CylonContext> &ctx,
                                   const std::shared_ptr<arrow::Table> &table,
                                   const int32_t col_idx,
                                   const ITER &indices_begin,
                                   const ITER &indices_end,
                                   int64_t size,
                                   arrow::ArrayVector &array_vector) {
  std::shared_ptr<arrow::Array> destination_col_array;
  arrow::Status ar_status = cylon::util::copy_array_by_indices(indices_begin, indices_end, size,
                                                               table->column(col_idx)->chunk(0),
                                                               destination_col_array, cylon::ToArrowPool(ctx));
  if (!ar_status.ok()) {
    LOG(FATAL) << "Failed while copying a column to the final table from tables."

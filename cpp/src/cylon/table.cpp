...
Status DistributedSort(std::shared_ptr<cylon::Table> &table,
  ...
  return DistributedSort(table, std::vector<int>{
                             sort_column
...
Status
Union(const std::shared_ptr<Table> &first, const std::shared_ptr<Table> &second, std::shared_ptr<Table> &out) {
...
// concat filtered tables
  const auto
      &concat_res = arrow::ConcatenateTables({
                                                 f_ltab, f_rtab
...
std::shared_ptr<BaseArrowIndex> Table::GetArrowIndex() {
  return base_arrow_index_;
}
...
Status
FromParquet(std::shared_ptr<cylon::CylonContext>
&ctx,
...
if (options.
IsConcurrentFileReads()
) {
std::vector<std::pair<std::future < Status>, std::thread>>
futures;
futures.
reserve(paths
.
...
return all_passed ?
Status::OK() : Status(cylon::IOError, "Failed to read the parquet files");
} else {
auto status = Status::OK();
for (
std::size_t kI = 0;

...
inline arrow::Status ArrowStatus(const cylon::Status &status) {
  return arrow::Status(static_cast<arrow::StatusCode>(status.get_code()),
                       status.get_msg());
}

class ProxyMemoryPool : public arrow::MemoryPool {
 public:
  explicit ProxyMemoryPool(cylon::MemoryPool *tx_memory) { this->tx_memory = tx_memory; }

  ~ProxyMemoryPool() override { delete tx_memory; }
  ...
  int64_t bytes_allocated() const override { return this->tx_memory->bytes_allocated(); }

  int64_t max_memory() const override { return this->tx_memory->max_memory(); }

  int64_t total_bytes_allocated() const override {
    return this->tx_memory->total_bytes_allocated();
  }

  int64_t num_allocations() const override { return this->tx_memory->num_allocations(); }

  std::string backend_name() const override { return this->tx_memory->backend_name(); }
